...
void loop() {
  ...
  if (run_mode ==1){ //findobject
    ...
    if(mode==11){
      ...
      if (green>blue &&green >red&&green -red >11){
        if (!LEDRunning) {
          digitalWrite(LED_green ,HIGH);
          startTime = millis(); // 動作開始時間を記録
          LEDRunning = true;
  ...
  if(run_mode == 2){ //approachwall
    ...
    if (mode_check2 ==4){ 
      if (!motorRunning) {//一番近い壁の方向を向く
        ...
        currentMillis = millis();
        if(currentMillis- startTime >= ((findMinIndex(Distance2, 12) + 6)%12+0.8) * 667-667*7){//ここの秒数を調節
          move(0);
          motorRunning = false;
          lastTime = millis();
          mode_check2 = 5;
