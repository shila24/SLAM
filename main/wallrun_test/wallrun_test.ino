...
const double point_finish =10; //最後の停止位置
const double point_sideMax =10; //最後の停止位置
const double point_sideMin =5; //最後の停止位置
...
//リスト
int pos[11]={ 10,20,45,60,80,90,105,125,140,165,170 };  //サーボの角度　45°から135°までの11個値を格納
...
void loop() {
  ...
  if (mode_check ==1){  
    ...
    switch(q){
      case 0 : 
        if(point_finish<N && R<L && point_sideMax>R && point_sideMin<R){
          q=1;
        }
        else if(point_finish<N && R>=L && point_sideMax>L && point_sideMin<L){
          q=1;
        }
        else if(point_finish<N && R<L && point_sideMax<R2){
          q=3;
        }
        else if(point_finish<N && R<L && point_sideMin>R2){
          q=6;
        }
        else if(point_finish<N && R>=L && point_sideMax<L){
          q=6;
        }
        else if(point_finish<N && R>=L && point_sideMin>L){
          q=3;
        }
        else if(N==0 || R==0 || L==0 || R1==0 || L1==0){
          q=1;
        }
        else if(N<point_finish){
          q=7;
        }
        else{
          if (!motorRunning) {
      ...
      case 13 :     //小前進
        if (!motorRunning) {
          ...
          currentMillis = millis();
          if(currentMillis- startTime>=600){
            move(0);
            q=0;  //状態
            lastTime = millis();
            mode_check =0;
