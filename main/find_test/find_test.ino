...
void loop() {
  ...
  if(mode==0){//遠い間は大回転して近い方へ
    if (getDistance(Object_Trig, Object_Echo)<point2&&getDistance(Wall_Trig, Wall_Echo) > point2+3){//壁じゃない
      ...
      if (!motorRunning) {
        ...
        currentMillis = millis();
        if(currentMillis- startTime>=338){
          move(0);
          distance_check3();
          motorRunning = false;
          if (Count3 ==0){
  ...
  if (mode ==1){ 
    if (!motorRunning) {//一番近い壁の方向を向く
      ...
      if(currentMillis- startTime>= a * 338){//ここの秒数を調節
        move(0);
        motorRunning = false;
        lastTime = millis();
        mode =2;
  ...
  if(mode==6){ 
    if (!motorRunning) {//逆回転して最小値へ
      ...
      currentMillis = millis();
      if(currentMillis- startTime>= (9-memory_n)  * 252){//ここの秒数を調節
        move(0);
        motorRunning = false;
        lastTime = millis();
        mode =3;
  ...
  if(mode==11){ 
    if (!motorRunning) {//逆回転して最小値へ
      ...
      currentMillis = millis();
      if(currentMillis- startTime>= (9-memory_n)  * 100){//ここの秒数を調節
        move(0);
        motorRunning = false;
        lastTime = millis();
        mode =8;
  ...
  if(mode==12){ 
     if (!motorRunning) {
      ...
      currentMillis = millis();
      if(currentMillis- startTime>=350){
        move(0);
        mode = 10;
      }
    }
