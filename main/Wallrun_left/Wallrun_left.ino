...
const double point_sideMax =20; //最後の停止位置
const double point_sideMin =10; //最後の停止位置
...
void loop() {
  ...
  if (mode_check ==1){  
    ...
    switch(q){
      case 0 : 
        if(point_finish<N && point_sideMax>R && point_sideMin<R){
          q=1;
        }
        else if(point_finish<N  && point_sideMax<R1){
          q=3;
        }
        else if(point_finish<N  && point_sideMin>R1){
          q=6;
        }
